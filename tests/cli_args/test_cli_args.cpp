...
int main() {
    ...
    /* --- --help only counts in FLAG position -------------------------------------- */
    /* Scanning every token would let a flag's value hijack the run: `--exec-at 5 -h` would
       print the usage text and exit 0 instead of running. */
    {
       ...
       exactly the reader who needed it. */
    {
        ...
        /* A named file in the working directory, not tmpfile(): on Windows tmpfile() wants the
           drive root and fails without the privileges to write there, and CI runs the host
