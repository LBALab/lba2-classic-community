...
# --- an --exec-at the run never reaches must not pass for a clean run -------------------
# It did not do what the command line asked for, so exiting 0 would be a silent no-op.
if timeout 60 "$LBA2_BIN" --headless --no-autosave --exec-at 500 "cube 100" --tick 20 --exit \
    >/dev/null 2>&1; then
