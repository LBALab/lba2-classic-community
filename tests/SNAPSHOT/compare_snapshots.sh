#!/bin/bash
# compare_snapshots.sh — Compare ASM vs CPP framebuffer output for a snapshot
#
# Usage: compare_snapshots.sh <snapshot.lba2snap> [asm_binary] [cpp_binary]
#
# Exit code: 0 = identical, 1 = different, 2 = error

set -euo pipefail

SNAP="${1:?Usage: compare_snapshots.sh <snapshot.lba2snap> [asm_binary] [cpp_binary]}"
ASM_BIN="${2:-./replay_snapshot_asm}"
CPP_BIN="${3:-./replay_snapshot_cpp}"

ASM_OUT=$(mktemp /tmp/snap_asm_XXXXXX.raw)
CPP_OUT=$(mktemp /tmp/snap_cpp_XXXXXX.raw)
trap "rm -f '$ASM_OUT' '$CPP_OUT'" EXIT

echo "=== Snapshot replay comparison ==="
echo "Snapshot: $SNAP"
