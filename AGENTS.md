...
| Running the engine at all | Pass `--user-dir <tmp>` so the run writes to a folder you own. Without it the run reads and writes the developer's real saves and settings, and a `--language` or a settings change persists there. `--profile <name>` nests inside it; `LBA2_USER_DIR` is the same for a wrapper. Comparing against a build that predates the flag needs `XDG_DATA_HOME`, which is the only lever both honour | docs/GAME_DATA.md, docs/PORTABILITY_PLAN.md |
| Paths, discovery, the writable side | Run `scripts/dev/dist_check.sh` before and after: five assertions across every install you own, non-zero exit. It caught a live defect the day it was written | docs/TESTING.md §6b |
