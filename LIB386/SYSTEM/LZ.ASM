...
fmt_expandlz_1	DB	"[ASM][ExpandLZ][1] Dst=%p Src=%p Size=%u MinBloc=%u", 10, 0
...
ExpandLZ		PROC \
		...
		; --- debug trace [ASM][ExpandLZ][1] params ---
		pushad
		push	MinBloc
		push	DecompSize
		push	Src
		push	Dst
		push	offset fmt_expandlz_1
		call	printf
		add	esp, 20
		popad
		; --- end debug trace ---
...
copie:							; ici on a 16 bits formant un offset
			...
			add	ebx, 2
			add	ecx, [BlockSize]			; on ajuste car pas de copie de moins de 2 (ou 3)
