...
TextInk			db	15
TextPaper		db	-1
TextDummy		dw	0	; to be able to read a DWORD
...
Font8X8			db	0, 0, 0, 0, 0, 0, 0, 0
			...
			db	24, 24, 24, 24, 255, 24, 24, 24
                        db      120, 0, 120, 12, 124, 204, 118, 0       ;24, 24, 31, 24, 31, 24, 24, 24
			...
			db	0, 0, 254, 108, 108, 108, 108, 0
                        db      120, 0, 120, 204, 204, 204, 120, 0      ;252, 204, 96, 48, 96, 204, 252, 0
...
fmt_affstrin_1	DB	"[ASM][AffString][1] x=%d y=%d str=%p", 10, 0
...
AffString		PROC \
		...
		; --- debug trace [ASM][AffString][1] params ---
		pushad
		push	string
		push	y
		push	x
		push	offset fmt_affstrin_1
		call	printf
		add	esp, 16
		popad
		; --- end debug trace ---
			...
			mov	ebp, TabOffLine[ebx*4]

			add	ebp, Log

			add	ebp, eax

			add	ebp, [SizeChar]	; pixel loop counter/index is
						; negative
...
Read_Char:
			...
			test	eax, eax
			jz	exit			; 0 is end of string

			mov	edi, ebp
			mov	dh, byte ptr[SizeChar]
...
line:
			...
			neg	ebx
col:
			mov	eax,dword ptr[TextInk]	; default to ink
