...
| [x] | `SVGA/SCALESPI.ASM` | `SVGA/SCALESPI.CPP` | `ScaleSprite` | Scale sprite with transparency | ASM fix: added `uses esi edi ebx` (callee-saved regs were clobbered, causing crash). CPP fix: `ScreenXMax`/`ScreenYMax` now match the ASM's exclusive upper-edge semantics. `tests/SVGA/test_scalespi.cpp` now replaces the placeholder/min-max-only checks with strict 1:1 framebuffer and `ScreenX/Y` bounds equivalence for minimal, hotspot, clipped-edge, fully clipped, and 100 deterministic random cases across the broader visible/off-screen domain. |
