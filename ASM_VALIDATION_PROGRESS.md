...
| Status | ASM File | CPP File | Function(s) | Description | Notes |
|--------|----------|----------|-------------|-------------|-------|
| [x] | `pol_work/POLY.ASM` | `pol_work/POLY.CPP` | INV64, SetFog, SetCLUT, Switch_Fillers, Fill_Poly, Fill_PolyClip, Triangle_ReadNextEdge, SetScreenPitch, Calc_TextureZ*SlopeFPU | Core polygon rendering pipeline | **ASM equiv** — 28 tests (13 CPP + 4 ASM-vs-CPP: INV64 300 rounds, SetFog 300 rounds, SetCLUT 300 rounds, Switch_Fillers 300 rounds; + Fill_PolyClip 300 rounds [~known discrepancy: ASM hardcodes ZBuf slope tables, CPP uses non-ZBuf], Fill_Poly 300 rounds [~same slope-table discrepancy; tests full entry point: dispatch setup → Jmp_Solid globals → asm_Fill_PolyClip → asm_Filler_Flat], Fill_Sphere 300 rounds, Line_A 300 rounds; + 6 TextureZ slope calculator unit tests: Calc_TextureZXSlopeFPU 300 rounds, Calc_TextureZGouraudXSlopeFPU 300 rounds, Calc_TextureZXSlopeZBufFPU 300 rounds, Calc_TextureZLeftSlopeFPU 300 rounds, Calc_TextureZGouraudLeftSlopeFPU 300 rounds, Calc_TextureZLeftSlopeZBufFPU 300 rounds — CPP regression tests against reference formula). POLY_JMP.ASM linked as ASM dep for ASM jump table verification. Fill_PolyFast cannot be tested directly (Watcom jmp-table dispatch to C-convention Jmp_Solid). Triangle_ReadNextEdge tested indirectly through Fill_PolyClip filler tail-call chain. |
...
| [x] | `pol_work/POLYLINE.ASM` | `pol_work/POLYLINE.CPP` | Line, Line_A, Line_ZBuffer, Line_ZBuffer_NZW | Polygon edge drawing | ASM equiv for Line() — 5 CPP + 4 ASM tests. Line_A also tested via test_poly (300 random rounds, non-zbuffer mode). |
