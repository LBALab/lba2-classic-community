...
| Status | ASM File | CPP File | Function(s) | Description | Notes |
|--------|----------|----------|-------------|-------------|-------|
| [~] | `OBJECT/AFF_OBJ.ASM` | `OBJECT/AFF_OBJ.CPP` | `ObjectDisplay`, `BodyDisplay`, `TestVisibleI`, `TestVisibleF`, `QuickSort`, `QuickSortInv` | Full 3D object rendering pipeline | Partial ASM ≡ CPP coverage: `tests/OBJECT/test_aff_obj.cpp` now validates `TestVisibleI`, `QuickSort`, and `QuickSortInv`. `TestVisibleI` is covered with fixed cases, hidden-point/degenerate edge cases, and randomized projected-point stress against the shared CPP `TestVisible` logic, while also checking that both the CPP and ASM `Obj_ListProjectedPoints` buffers remain unchanged. The sort helpers keep deep full-array comparisons including guard words across fixed cases, edge cases, monotonic random stress, and ASM-valid unordered random stress. Fixed CPP recursion to exclude the pivot (`left - 1`, `left + 1`), and exported ASM `QuickSort`/`QuickSortInv` plus an internal C-callable wrapper for `TestVisibleI` testing. `TestVisibleF` is an alternate ASM float-path helper with no direct CPP counterpart in `AFF_OBJ.CPP`, so it still needs a dedicated CPP mirror before it can be covered as a strict equivalence test. Fully arbitrary unordered inputs are not valid for these original ASM quicksort helpers because some pivot orders recurse on the same range; full object-display coverage still needs a complete 3D fixture. |
