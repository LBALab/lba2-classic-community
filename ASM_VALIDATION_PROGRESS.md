...
| Status | ASM File | CPP File | Function(s) | Description | Notes |
|--------|----------|----------|-------------|-------------|-------|
| [~] | `OBJECT/AFF_OBJ.ASM` | `OBJECT/AFF_OBJ.CPP` | `ObjectDisplay`, `BodyDisplay`, `BodyDisplay_AlphaBeta`, `TestVisibleI`, `TestVisibleF`, `QuickSort`, `QuickSortInv` | Full 3D object rendering pipeline | `tests/OBJECT/test_aff_obj.cpp` now passes with the original simple render fixture, a broader mixed-polygon object fixture, textured solid-triangle, textured flat-triangle, textured gouraud-triangle, textured perspective-correct flat-triangle, textured flat-quad, and environment-mapped solid-triangle, gouraud-triangle, and flat-triangle fixtures. Coverage now includes visible and clipped end-to-end cases for `BodyDisplay`, `BodyDisplay_AlphaBeta`, and `ObjectDisplay`, multigroup `ObjectDisplay` cases that exercise a second group through both rotation and `TYPE_TRANSLATE` paths with mixed flat, solid, gouraud, triangle, and quad polygon dispatch, textured `ObjectDisplay` cases that drive the solid, flat, gouraud, quad-flat, and `POLY_TEXTURE_Z_FLAT` paths with real UVs, a texture table, and texture filler setup, and env-mapped `ObjectDisplay` cases that exercise the ASM env solid, env gouraud, and env flat branches end-to-end. The current env branch coverage uses `scale = 0`; non-zero env scale still appears to be a separate uncovered mismatch and should be debugged with a dedicated reproducer instead of folded into these branch tests. The render fixtures compare return values, screen bounds, `Obj_ListRotatedPoints`, `Obj_ListProjectedPoints`, `ListLights`, `ListSort`, `TabMat`, `ListFillPoly`, `PosXWr/Y/Z`, and the rendered framebuffer. Root fixes required for parity were: `InitMatrixTransF` storing translation slots with float representation, `ObjectDisplay` placing face-light data at `ListLights + NbPoints` rather than `ListLights + NbPoints * 2`, restoring `Triangle_TextureZ_Flat`/`Quad_TextureZ_Flat` to populate perspective `Pt_W` values instead of zeroing them, the AFF_OBJ env handlers rotating per-vertex normals into env UVs while preserving the ASM-visible `X0/Y0/Z0` side effect, and the test harness installing Watcom-compatible `RotatePoint`/`RotatePointNoMMX` thunks so the ASM env path can call the C++ 3D callbacks correctly. Fully arbitrary unordered inputs are still invalid for the original ASM quicksort helpers because some pivot orders recurse on the same range, so unordered sort stress remains restricted to the ASM-valid domain. |
