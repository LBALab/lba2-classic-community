...
| [~] | `pol_work/POLYTEXZ.ASM` | `pol_work/POLYTEXZ.CPP` | Filler_TextureZ, Fill_Init_Perspective, ... (20+ funcs) | Perspective-correct texture fill | **ASM equiv** — 3 CPP + 1 ASM static test pass. 30-round ASM random stress (no-crash only). x87 vs C `double` precision differences prevent strict byte comparison. |
| [~] | `pol_work/POLYTZF.ASM` | `pol_work/POLYTZF.CPP` | Filler_TextureZFogSmooth, ... (3 fillers) | TextureZ + smooth fog variants | **ASM equiv** — 3 CPP + 2 ASM tests (static + 30-round no-crash stress). x87 vs C `double` precision prevents strict comparison. |
