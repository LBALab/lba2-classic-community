...
| [~] | `pol_work/POLY_JMP.ASM` | `pol_work/POLY_JMP.CPP` | Jmp_Solid, Jmp_Trame, Jmp_Transparent, Jmp_Gouraud, ... (80+ dispatch functions) | Polygon type dispatcher | CPP tested via Fill_Poly dispatch (7 tests inc. random). No ASM equiv — dispatch table too complex. |
...
| [~] | `pol_work/POLYFLAT.ASM` | `pol_work/POLYFLAT.CPP` | Filler_Flat, Filler_Trame, Filler_Transparent, ... (10 fillers) | Solid-color polygon fill | **ASM equiv** — 12 CPP + ASM filler tests. **FIXED**: Fill_Color must have color in both bytes of word (Jmp_Solid pattern `color | color<<8`). |
| [x] | `pol_work/POLYGOUR.ASM` | `pol_work/POLYGOUR.CPP` | Filler_Gouraud, Filler_Dither, ... (18 fillers) | Gouraud + dither shading | **ASM equiv** — 6 CPP + 3 ASM filler tests (basic, uniform, 30 random). |
| [x] | `pol_work/POLYGTEX.ASM` | `pol_work/POLYGTEX.CPP` | Filler_TextureGouraud, Filler_TextureDither, ... (8 fillers) | Texture mapping with Gouraud shading | **ASM equiv** — 5 CPP + 3 ASM filler tests. **FIXED**: sub-pixel factor was `0x10000 - frac` (off-by-1); ASM uses `frac ^ 0xFFFF`. Also fixed S32 overflow in sub-pixel UV adjustment (needs S64 multiply). |
...
| [~] | `pol_work/POLYTEXT.ASM` | `pol_work/POLYTEXT.CPP` | Filler_Texture, Filler_TextureFlat, ... (18 fillers) | Texture-mapped polygon fill | **ASM equiv** — 4 CPP + 3 ASM filler tests. **FIXED**: S32 multiplication overflow in sub-pixel UV correction (needs S64 multiply like ASM's 64-bit `imul`). |
| [x] | `pol_work/POLYTEXZ.ASM` | `pol_work/POLYTEXZ.CPP` | Filler_TextureZ, Fill_Init_Perspective, ... (20+ funcs) | Perspective-correct texture fill | **ASM equiv** — 3 CPP + 2 ASM strict tests (static + 30 random). **FIXED**: replaced `double` math with inline x87 asm (`fild/fdivr/fmul/fistp`) matching ASM FPU instruction sequence exactly. |
| [x] | `pol_work/POLYTZF.ASM` | `pol_work/POLYTZF.CPP` | Filler_TextureZFogSmooth, Filler_TextureZFogSmoothZBuf, Filler_TextureZFogSmoothNZW | TextureZ + smooth fog variants | **ASM equiv** — 3 CPP + 6 ASM strict tests (static + 300 random each for all 3 fillers). **FIXED**: rewrote all 3 CPP fillers from affine to perspective-correct with inline x87 asm, fog CLUT, ZBuf perspective correction, Z-buffer rotated comparison (ror8 + add/adc). Fixed sub-pixel correction accumulation bug. ZBuf/NZW tests verify both framebuffer and Z-buffer byte-for-byte. |
| [~] | `pol_work/POLYTZG.ASM` | `pol_work/POLYTZG.CPP` | Filler_TextureZGouraud, ... (6 fillers) | TextureZ + Gouraud variants | CPP tested via Fill_Poly (3 tests inc. random). ASM blocked: filler calls `[Fill_Init_Perspective]` as Watcom-convention function pointer — needs full ABI shim. |
