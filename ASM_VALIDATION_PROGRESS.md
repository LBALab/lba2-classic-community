...
| Status | ASM File | CPP File | Function(s) | Description | Notes |
|--------|----------|----------|-------------|-------------|-------|
| [~] | `OBJECT/AFF_OBJ.ASM` | `OBJECT/AFF_OBJ.CPP` | `ObjectDisplay`, `BodyDisplay`, `BodyDisplay_AlphaBeta`, `TestVisibleI`, `TestVisibleF`, `QuickSort`, `QuickSortInv` | Full 3D object rendering pipeline | `tests/OBJECT/test_aff_obj.cpp` now passes with a concrete one-group 3D body fixture covering visible and clipped end-to-end cases for `BodyDisplay`, `BodyDisplay_AlphaBeta`, and `ObjectDisplay`, while retaining strict helper coverage for `TestVisibleI`, `TestVisibleF`, `QuickSort`, and `QuickSortInv`. The render fixture compares return values, screen bounds, `Obj_ListRotatedPoints`, `Obj_ListProjectedPoints`, `ListLights`, `ListSort`, `TabMat`, `ListFillPoly`, `PosXWr/Y/Z`, and the rendered framebuffer. Root fixes required for parity were: `InitMatrixTransF` storing float translations instead of raw integer bit patterns, and `ObjectDisplay` placing face-light data at `ListLights + NbPoints` rather than `ListLights + NbPoints * 2`. Fully arbitrary unordered inputs are still invalid for the original ASM quicksort helpers because some pivot orders recurse on the same range, so unordered sort stress remains restricted to the ASM-valid domain. |
