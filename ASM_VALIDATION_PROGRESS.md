...
| Status | ASM File | CPP File | Function(s) | Description | Notes |
|--------|----------|----------|-------------|-------------|-------|
| [~] | `OBJECT/AFF_OBJ.ASM` | `OBJECT/AFF_OBJ.CPP` | `ObjectDisplay`, `BodyDisplay`, `BodyDisplay_AlphaBeta`, `TestVisibleI`, `TestVisibleF`, `QuickSort`, `QuickSortInv` | Full 3D object rendering pipeline | `tests/OBJECT/test_aff_obj.cpp` now passes with the original simple render fixture, a broader mixed-polygon object fixture, and a textured flat-triangle fixture. Coverage now includes visible and clipped end-to-end cases for `BodyDisplay`, `BodyDisplay_AlphaBeta`, and `ObjectDisplay`, multigroup `ObjectDisplay` cases that exercise a second group through both rotation and `TYPE_TRANSLATE` paths with mixed flat, solid, gouraud, triangle, and quad polygon dispatch, and a textured `ObjectDisplay` case that drives the flat texture path with real UVs, a texture table, and texture filler setup. The render fixtures compare return values, screen bounds, `Obj_ListRotatedPoints`, `Obj_ListProjectedPoints`, `ListLights`, `ListSort`, `TabMat`, `ListFillPoly`, `PosXWr/Y/Z`, and the rendered framebuffer. Root fixes required for parity were: `InitMatrixTransF` storing translation slots with float representation, and `ObjectDisplay` placing face-light data at `ListLights + NbPoints` rather than `ListLights + NbPoints * 2`. Fully arbitrary unordered inputs are still invalid for the original ASM quicksort helpers because some pivot orders recurse on the same range, so unordered sort stress remains restricted to the ASM-valid domain. |
