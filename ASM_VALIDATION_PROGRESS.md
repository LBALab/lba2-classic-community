...
| Status | ASM File | CPP File | Function(s) | Description | Notes |
|--------|----------|----------|-------------|-------------|-------|
| [~] | `OBJECT/AFF_OBJ.ASM` | `OBJECT/AFF_OBJ.CPP` | `ObjectDisplay`, `BodyDisplay`, `TestVisibleI`, `TestVisibleF`, `QuickSort`, `QuickSortInv` | Full 3D object rendering pipeline | Partial ASM ≡ CPP coverage: `tests/OBJECT/test_aff_obj.cpp` now validates `TestVisibleI`, `TestVisibleF`, `QuickSort`, and `QuickSortInv`. `TestVisibleI` is covered against the shared CPP `TestVisible` logic, while `TestVisibleF` is covered against a dedicated test-local CPP mirror that matches the validated floating-point winding semantics from `pol_work/TESTVUEF.CPP` (`ROUND(cross) >= 0`). Both visibility helpers have fixed cases, hidden-point/degenerate edge cases, and randomized projected-point stress, and the tests verify that both the CPP and ASM `Obj_ListProjectedPoints` buffers remain unchanged. The sort helpers keep deep full-array comparisons including guard words across fixed cases, edge cases, monotonic random stress, and ASM-valid unordered random stress. Fixed CPP recursion to exclude the pivot (`left - 1`, `left + 1`), and exported/internal C-callable ASM wrappers for `TestVisibleI`, `TestVisibleF`, `QuickSort`, and `QuickSortInv` for direct testing. Fully arbitrary unordered inputs are not valid for these original ASM quicksort helpers because some pivot orders recurse on the same range; full object-display coverage still needs a complete 3D fixture. |
