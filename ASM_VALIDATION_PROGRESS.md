...
| [x] | `3D/PRLIISO.ASM` | `3D/PRLIISO.CPP` | `ProjectListIso` | Batch isometric projection | `tests/3D/test_prliiso.cpp` now covers multi-point batches, variable centres/origins, screen-bounds globals, and in-place source mutation. Fixed CPP parity bugs: the projection math must use 32-bit intermediates like ASM, and the per-vertex sort value must be written back as a full 32-bit store over the source `Z`/`Grp` pair. Note: `n == 0` is not a valid input domain for the ASM implementation and is intentionally not tested. |
...
| [x] | `3D/REGLE3.ASM` | `3D/REGLE3.CPP` | `RegleTrois`, `BoundRegleTrois` | Linear interpolation (rule of three) ± bounds | `tests/3D/test_regle3.cpp` now covers zero and negative `NbStep`, negative and out-of-range `Step`, large signed values, and 200 deterministic LCG stress rounds for both unclamped and clamped interpolation paths. |
