...
| [x] | `SVGA/GRAPH.ASM` | `SVGA/GRAPH.CPP` | `AffGraph`, `GetBoxGraph` | Graphics drawing and box calculation | `tests/SVGA/test_graph.cpp` now covers basic, positive-hotspot, and negative-hotspot `GetBoxGraph` cases, including the signed hotspot path that previously differed because the CPP code zero-extended `HotX`/`HotY` instead of matching the ASM `movsx` behavior. |
