...
| [~] | `pol_work/POLYTEXZ.ASM` | `pol_work/POLYTEXZ.CPP` | Filler_TextureZ, Fill_Init_Perspective, ... (20+ funcs) | Perspective-correct texture fill | **ASM equiv** — 3 CPP + 1 ASM strict static + 30-round ASM no-crash stress. **FIXED**: changed `double` to `long double` and `ROUND` to `lrintl` to match x87 80-bit precision. Remaining FP rounding differences with varying W. |
| [~] | `pol_work/POLYTZF.ASM` | `pol_work/POLYTZF.CPP` | Filler_TextureZFogSmooth, ... (3 fillers) | TextureZ + smooth fog variants | **ASM equiv** — 3 CPP + 2 ASM tests (approximate comparison allows <20% pixel diff from FP rounding + 30-round no-crash stress). |
