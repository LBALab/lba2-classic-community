...
| [~] | `pol_work/POLYTEXT.ASM` | `pol_work/POLYTEXT.CPP` | Filler_Texture, Filler_TextureFlat, ... (18 fillers) | Texture-mapped polygon fill | **ASM equiv** — 4 CPP + 41 ASM filler tests (all 18 variants: static + 300-round random stress). 30 tests pass; remaining fail due to pre-existing CPP bugs: inverted Z-buffer comparison (`>=` vs ASM's `<` / `jb`) in NZW and ChromaKey+ZBuf variants, and sub-pixel UV correction applied to outer-scope accumulators instead of local copies. **Previously FIXED**: S32 multiplication overflow in sub-pixel UV correction (needs S64 multiply like ASM's 64-bit `imul`). |
