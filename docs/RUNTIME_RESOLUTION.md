...
| **Phase 2** | Display options submenu in `GAMEMENU.CPP` — `MENU_ID_DISPLAY` entry alongside Controls. Opens a self-contained modal (`SOURCES/RES_MENU.CPP`) with two entries by default: Classic 4:3 (640×480) and a Widescreen entry computed from the current display's aspect ratio via `Window_GetDisplayDimensions`, snapped to `W%8==0`. Selection triggers `Res_Switch` + `Res_BeginRevertCountdown`, then bubbles out of `OptionsMenu` so MainLoop ticks the existing Phase 0 keep/revert dialog immediately. Headless captures fall through to Classic-only (no display dims). `ui display <path>` capture verb + golden lock the rendering. Advanced list / Fullscreen / Vsync siblings deliberately deferred — keeps v1 to a two-button shape until A4/A5 unblocks HD modes. | ✅ shipped (this PR) |
