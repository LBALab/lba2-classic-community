...
[^diskfunc-cpp]: [SOURCES/DISKFUNC.CPP](../../../SOURCES/DISKFUNC.CPP), `LoadScene`, the `if` around `FadeToBlackAndSamples`.
[^object-cpp]: [SOURCES/OBJECT.CPP](../../../SOURCES/OBJECT.CPP), `ChangeCube`, the `if (!FlagLoadGame)` block after `RestartPerso`.
[^gamemenu-cpp]: [SOURCES/GAMEMENU.CPP](../../../SOURCES/GAMEMENU.CPP), the `switch (DetailLevel)` that assigns `FlagDrawHorizon`; [SOURCES/GLOBAL.CPP](../../../SOURCES/GLOBAL.CPP) gives it the default.
[^original]: Commit 333929ab, the initial import, [SOURCES/DISKFUNC.CPP](../../../SOURCES/DISKFUNC.CPP), the same condition in `LoadScene`.
[^transitions-doc]: [docs/TRANSITIONS.md](../../TRANSITIONS.md), "The core principle: black brackets the load".
[^control-cpp]: [SOURCES/CONTROL.CPP](../../../SOURCES/CONTROL.CPP), the comment above `Control_DescribeState`, the `DetailLevel 3 -> 0` row of the first sweep, and the last line of the `settings.` block; [SOURCES/RECORD.CPP](../../../SOURCES/RECORD.CPP), `mode_line_ignored`, which carries no `settings.` key, and the `replay ended` line, which carries no mode count.
