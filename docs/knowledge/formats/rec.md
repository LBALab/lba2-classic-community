...
[^control-cpp]: SOURCES/CONTROL.CPP, `Control_StateDigest`, the actor loop and the two variable loops to `MAX_VARS_GAME` and `MAX_VARS_CUBE`; scripts/dev/dump_recording.py, `tele_layout`, is the reader that anchors from the end. For the `settings.` row, the sweep table in the comment above `Control_DescribeState` and the block it writes.
