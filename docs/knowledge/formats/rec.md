...
| `settings.` | `FollowCamera` and nine `FollowCam` tuning keys | compared, and not installed: a replay reports the difference and runs on its own config, so an operator whose auto-camera setting differs from the recording's sees a divergence at tick 0 that is a config mismatch. Written by the harness, not the recorder. The architecture call on the table is to move these to installed beside `bindings.` and `state.`; read this row at `as_of`. |
