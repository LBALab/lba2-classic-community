...
| One residue class under a pinned step | Under a pinned step the game clock advances only through `ManageTime` banking multiples of the step, and changes otherwise only by the direct writers, which assign. So between two installs every delta between consecutive tick clocks is a multiple of the step, and a run holds a single residue class modulo the step whose value is whatever the last install left. Checked on the five fixture recordings in the tree: one residue each, every non-step delta a multiple of 16. It is a replay oracle that needs no second run: a tick clock off its run's residue class is wall time or an unrecorded install.[^timer-cpp] |
