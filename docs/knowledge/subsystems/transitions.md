...
[^perso-cpp]: SOURCES/PERSO.CPP, `MainLoop`: the `NewCube != -1` block at the top with its `#ifdef DEMO` half, the one `Control_TickHook()` call after it, the `DoTrack` and `DoLife` calls in the per-object loop, the `AffScene(FirstTime)` call at the end of the iteration, and the `goto startloop` inside the per-object loop.
[^control-cpp]: SOURCES/CONTROL.CPP, the comment above `s_uncarried` and the `DIGEST_CARRIED` lines in `Control_StateDigest`.
[^record-cpp]: SOURCES/RECORD.CPP, `REPLAY_END_STALL` in the `T_ReplayEnd` enum and its comment, and the `s_polls` and `s_ticks` counters.
