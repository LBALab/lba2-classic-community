...
| [RECORDING_RESEARCH.md](plan/RECORDING_RESEARCH.md) | Research | Where a Doom 3 style recording system would attach: the four layers it could sit at, why the polled device state is the only complete one, why the index is input polls rather than ticks, and what a recording carrying its own state hash buys for fixtures, attract demos and bug reports. |
