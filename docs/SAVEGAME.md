...
**Reader (canonical-first).** `LoadContexte` reads the canonical 32-bit wire layout directly, with no predictive stride heuristic. A save this port wrote *before* the portable writer landed used the host-native 64-bit stride; the reader detects that only as a fallback: if the canonical read fails per-object `IndexFile3D` validation it rewinds, retries the native stride, and on success warns that the save will migrate to the portable layout the next time it is written. `LBA2_SAVE_LOAD_ABI` forces a path for tests (`32` = wire only, `64` = native only). A `wire32_abi` flag from the object loop selects the matching `T_EXTRA` / `S_PART_FLOW` read (single-writer assumption: one program wrote the whole file, so all three share the same ABI). The `IndexFile3D` validation is what keeps a wrong-stride read from corrupting state: it returns `SAVELOAD_CTX_ERR` before any runtime state is replaced, never a `LoadFile3D(garbage)` SIGSEGV in `InitLoadedGame`.
...
| Per-object stride | `276` (canonical 32-bit wire) with a legacy native fallback | **Canonical-first** | Read the 32-bit wire directly; on `IndexFile3D` validation failure rewind and retry the native stride (a pre-fix port-64 save), warn, migrate on next save. Both fail: `SAVELOAD_CTX_ERR`. `LBA2_SAVE_LOAD_ABI` (`32`/`64`) forces a path. Closes the `LoadFile3D(garbage)` SIGSEGV in `InitLoadedGame`. |
