...
**The loop:** `S32 MainLoop()` — `SOURCES/PERSO.CPP:438`, body is `while (TRUE)` at `:478`.
...
- **Public API** (engine-facing, backend-agnostic): `LIB386/H/AIL/{SAMPLE,STREAM,CD,MIDI,VIDEO_AUDIO}.H`
  — `PlaySample`, `OpenStream/PlayStream`, `OpenCD/PlayCD`, `StartVideoAudio/PushVideoAudio`, etc.
...
the only live SDL calls in this group (`message_box` has no caller — dropped, §3.2); raw file IO does **not** need to enter the PAL (it is
