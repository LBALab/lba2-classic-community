...
| **camtrace** &lt;0\|1&gt; | Log the Auto camera's angle state on every follow-cam update: hero `Beta`, `AddBetaCam`, `BetaCam` and the target it is lerping toward, how far `BetaCam` moved that frame, the re-engage countdown, whether a manual orbit is driving, and whether the hero is translating. Paired with `camnudge` it turns "the camera snaps when I touch the stick" into numbers. |
