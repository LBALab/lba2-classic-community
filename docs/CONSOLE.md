...
- **CVars**: a tunable that a module owns is declared once in that module's settings table and registered from there (`Settings_RegisterCvars`, see [CONFIG.md](CONFIG.md#how-a-key-is-declared)), which carries its range so the console cannot set a value the config would reject. `Console_RegisterCvar` in `CONSOLE_CMD.CPP` remains for a global with no owning module, and registers an unbounded cvar; `Console_RegisterCvarRanged` takes the bounds directly. Wire cvars to existing globals rather than duplicating state.
