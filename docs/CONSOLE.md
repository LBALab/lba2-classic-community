...
| **rec** start &lt;path&gt; \| stop \| play &lt;path&gt; \| info [path] | Record the session's input, the console commands driving it, the binding tables and a per-tick digest of simulation state, then replay it and report the first tick where the two stop matching. `start` snapshots and reloads first, so the recording begins from a state a replay can reach the same way; `info` with no path reports the running session, and with one compares that file's mode lines against this run. Needs `--fixed-dt`: on the host-sampled clock a replay is not exact. See [RECORDING.md](RECORDING.md). |
