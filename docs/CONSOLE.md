...
| **input** &lt;flag[,flag...]&gt; [frames] \| **input off** | Hold input bits for N frames (default 1), so the harness can drive the hero without a keyboard: `input up 60` walks forward, `input up,left 30` walks while turning. Flags: `up down left right action search throw inventory holomap` (or a raw `0xNNN` mask). Movement is held state, so a multi-frame hold walks/turns. |
