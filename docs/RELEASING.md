...
| **Artifact naming** | `<exe>-<version>-<platform>-<arch>.<ext>`, e.g. `lba2cc-0.11.0-windows-x64.zip`, `lba2cc-0.11.0-linux-x86_64.tar.gz`, `lba2cc-0.11.0-macos-arm64.dmg`, `lba2cc-0.11.0-android-arm64-v8a.apk`. The AppImage's platform label is `anylinux` (`lba2cc-0.11.0-anylinux-x86_64.AppImage`) and it ships a `.zsync` companion alongside it for delta self-update. Note the update channel resolves to the newest **stable** release, not the rolling one: `latest` in the `gh-releases-zsync` update information is a reserved keyword meaning "newest non-prerelease", not a tag name, so the `prerelease: true` rolling release is skipped. The `.zsync` files on the rolling release are therefore never read. Pointing self-update at the rolling channel would mean `latest-pre`, which is deliberately not used (see [Rolling latest pre-release](#rolling-latest-pre-release)). |
...
**AppImage self-update does not follow this channel, by design.** The update
information baked in by `make-appimage.sh` is
`gh-releases-zsync|<owner>|<repo>|latest|*<arch>.AppImage.zsync`, and `latest`
there is a reserved keyword meaning "newest non-prerelease" rather than the tag
named `latest`. Both the C++ `AppImageUpdate` and the Rust `appimageupdatetool`
that `self-updater.hook` actually downloads implement it that way, so an
installed AppImage updates to the newest stable tag and never to a rolling
build. Retargeting it would mean `latest-pre`, but the same script builds
tagged releases, so tagged AppImages would then also self-update onto
pre-releases. Left alone deliberately.
